...
public class MyStringTools {
	...
	/***
	 * Generate random string for Numero_Etranger
	 *
	 * @return String[]
	 */
	def getRandomNumber(long min, long max) {
		Random random = new Random()
		return (Math.abs(random.nextLong() % (max - min + 1))) + min
	}

	/***
	 def String generateRandomDigits(int length) {
	 String digits = '0123456789'  // Seulement des chiffres
	 Random rand = new Random()
	 StringBuilder randomString = new StringBuilder()
	 // Créer une chaîne de longueur spécifiée
	 for (int i = 0; i < length; i++) {
	 randomString.append(digits.charAt(rand.nextInt(digits.length())))
	 }
	 return randomString.toString()
	 }
	 */

	String genererChaineAleatoire(int longueur) {
		def caracteres = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'  // Inclut des lettres et des chiffres
