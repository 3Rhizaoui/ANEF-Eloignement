...
GlobalVariable.DriverName = DriverName
  println 'DriverName est :::' + DriverName
...
if (Profiles == 'Usager') {
	switch (DriverName) {
		...
		case 'CHROME_DRIVER':
			...
			'Navigation à URL Préprode  et Création MDP'
			if (EnvExec == 'URL_ANEFPreProd') {WebUI.navigateToUrl (GlobalVariable.URL_ANEFPreProd)}
			'Navigation à URL Env DEV  et Création MDP'
			if (EnvExec == 'URL_ANEFdevfenv5') {WebUI.navigateToUrl (GlobalVariable.URL_ANEFdevfenv5)}
		...
		case 'HEADLESS_DRIVER':
			...
			WebUI.maximizeWindow()
			"Appel de l'action Réutilisable pour contourner l'erreur lié à la confidentialité"
			//WebUI.callTestCase(findTestCase('Test Reutilisable/Main/ANEF_Eloignement_V_Nawres/Erreur_lie_a_la_confidentialite'), [:], FailureHandling.OPTIONAL)
			WebUI.delay(5)
			"Vérification de l'étape :"
			//WebUI.verifyElementVisible(findTestObject('null'), FailureHandling.STOP_ON_FAILURE)
			break;
...
if (Profiles == 'Agent') {
	'Navigation à URL  Qualification et Création MDP'
	if (EnvExec == 'URL_ANEFQualif') { WebUI.openBrowser (GlobalVariable.URLAdminNATALI)}
	'Navigation à URL Préprode  et Création MDP'
